...
def test_force_and_reset_depeg(
    ...
    assert price_info['compliance'] == 1
    assert price_info['stability'] == 1
    assert price_info['triggeredAt'] == 0
    assert price_info['depeggedAt'] == 0
    ...
    assert price_info_depeg_init['compliance'] == 0
    assert price_info_depeg_init['stability'] == 0
    ...
    tx = usdc_feeder.forceDepegForNextPriceInfo({'from':instanceOperator})
    ...
    assert price_info['compliance'] == 3 # failedMultiple
    assert price_info['stability'] == 3 # depegged
    ...
    # reset depeg state
    tx = usdc_feeder.resetDepeg({'from':instanceOperator})
    print('tx.info() {}'.format(tx.info()))
    ...
    assert price_info_depeg_reset['compliance'] == 0
    assert price_info_depeg_reset['stability'] == 0
...
def test_price_data_trigger_and_depeg(usdc_feeder: UsdcPriceDataProvider):
    ...
    for i, data in enumerate(USDC_CHAINLINK_DATA_TRIGGER_AND_DEPEG):
        ...
        # 7 1660186401  99900000 at recovery, but too late -> depeg
            ...
            assert price_info['stability'] == 3 # Stable
        # 8 1660200000  99700000 below recovery and above trigger
...
# def test_feed_manipulation_disabled_on_mainnet(usdc_feeder: UsdcPriceDataProvider):
...
#     # check IPriceDataProvider.forceDepegForNextPriceInfo
#     with brownie.reverts("ERROR:ADP-001:NOT_TEST_CHAIN"):
#         usdc_feeder.forceDepegForNextPriceInfo()

#     # check IPriceDataProvider.resetDepeg
#     with brownie.reverts("ERROR:ADP-001:NOT_TEST_CHAIN"):
#         usdc_feeder.resetDepeg()


# def test_force_and_reset_depeg(usdc_feeder: UsdcPriceDataProvider):
